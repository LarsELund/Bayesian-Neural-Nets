...
class BayesianLinear(nn.Module):
    def __init__(self, in_features, out_features,num_transforms):
        ...
        ### define the flow
        self.flow = PropagateFlow('Planar',n*m + m,num_transforms)
    ...
    def forward(self, input, sample=False, calculate_log_probs=False):
        ...
        z_new,logdet = self.flow(z_vector)
        W = z_new[:len(z_new)-len(bias)].reshape(weight.shape)
        B = z_new[-len(bias):]
        ...
        if self.training or calculate_log_probs:
            ...
            self.log_variational_posterior = self.weight.log_prob(weight) \
                + self.bias.log_prob(bias) - logdet
        else:
...
class BayesianNetwork(nn.Module):
    def __init__(self):
        super().__init__()
        self.l1 = BayesianLinear(28 * 28, 400,num_transforms = 4)
        self.l2 = BayesianLinear(400, 600,num_transforms = 4)
        self.l3 = BayesianLinear(600, 10,num_transforms = 4)
...
print('Took',time/epochs,'seconds/epoch on',DEVICE) 

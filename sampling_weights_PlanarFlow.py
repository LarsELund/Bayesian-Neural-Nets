...
mnist_loader = torch.utils.data.DataLoader(
    datasets.FashionMNIST('./fmnist', train=False, download=False, transform=transforms.ToTensor()), batch_size=5, shuffle=False)
...
class Gaussian(object):
    ...
    def sample(self):
        self.epsilon = self.normal.sample(self.rho.size()).to(DEVICE)
    ...
    def standard_normal_logprob(self,input):
        ##need this to compute log qo(zo)
        return(-0.5*input**2-0.5*torch.log(torch.tensor(2*math.pi))).sum()


class ScaleMixtureGaussian(object): #prior
...
class PlanarFlow(nn.Module):
    def __init__(self,n,m):
        ...
        self.u = nn.Parameter(parameter_init(-0.01,0.01,n*m + m))  
        self.w = nn.Parameter(parameter_init(-0.01,0.01,n*m + m)) 
...
class BayesianLinear(nn.Module):
    def __init__(self, in_features, out_features,num_flows):
        super().__init__()
        ...
        self.flows = nn.ModuleList([PlanarFlow(in_features,out_features) for i in range(num_flows)])
    ...
    def forward(self, input, sample=False, calculate_log_probs=False):
        if self.training or sample:
            ...
            ###run the weights and biases through the flow
            ###need to convert them into one vector
        z_vector = torch.cat((weight.flatten(),bias.flatten())) 
        logdet = 0
        for f,flow in enumerate(self.flows):
            if f == 0:
                z_new,logdet = flow(z_vector)
            else:
                z_new,logdet = flow(z_new)
            logdet += logdet

        weight = z_new[:len(z_new)-len(bias)].reshape(weight.shape)
        bias = z_new[-len(bias):]    
        ...
        if self.training or calculate_log_probs:
            ...
            logZ0 = self.weight.standard_normal_logprob(self.weight.epsilon) \
                + self.bias.standard_normal_logprob(self.bias.epsilon)
            
            repar_trick_derivative = torch.log(self.weight.sigma).sum() \
                + torch.log(self.bias.sigma).sum() #derivative of the reparametrization trick
            
            self.log_variational_posterior = logZ0 - repar_trick_derivative - logdet
        else:
...
class BayesianNetwork(nn.Module):
    def __init__(self):
        super().__init__()
        self.l1 = BayesianLinear(28 * 28, 400,num_flows = 3)
        self.l2 = BayesianLinear(400, 600,num_flows = 3)
        self.l3 = BayesianLinear(600, 10,num_flows = 3)
...
def sigmoid(x): #more numerically stable to use expit function
    return expit(x)

...
for i in range(0, k):
    ...
    plt.hist(net.l1.alpha.view(-1).cpu().detach().cpu().numpy(), bins=20,  color = "skyblue", ec="skyblue")
    plt.show()
    plt.close()
...
np.savetxt('RMSE_base' +'.txt',rmse2, delimiter=',',fmt='%s')
np.savetxt('predicted_alphas_base' +'.txt',pa, delimiter=',',fmt='%s')
np.savetxt('true_weights_base' +'.txt',tw, delimiter=',',fmt='%s')
